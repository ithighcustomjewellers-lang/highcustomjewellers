        ...
        <div class="col-lg-6">
            <div class="card border-0 shadow-lg rounded-4">
                ...
                <div class="card-body p-4">
                    ...
                    <form id="userMasteremailForm" method="POST" enctype="multipart/form-data">
                        @csrf
                        <!-- Basic Info Row -->
                        ...
                        <div class="d-flex gap-3 mt-4 flex-wrap">
                            ...
                            <a href="{{ route('admin-usersMaster-index') }}" class="btn btn-light btn-lg px-5 rounded-pill shadow-sm back-btn">
...
<style>
    ...
    .mobile-preview .btn, .desktop-preview .btn { display: inline-flex; align-items: center; justify-content: center; text-align: center; min-width: 130px; height: 42px; padding: 0 18px; border-radius: 50px;
            text-decoration: none; font-size: 14px; font-weight: 600; color: #fff; transition: all .25s ease;
    }
...
<script>
    ...
    function initializeActionLinks() {
        ...
        if (selectedActionLinks && selectedActionLinks.length > 0) {
            ...
            html = `<div class="text-muted small text-center py-2">No action links added. Click "Add Link" to add one.</div>`;
        }
    ...
    $(document).on('click', '.remove-action-link', function() {
        ...
        if (selectedActionLinks.length === 0) {
            $('#dynamicActionLinks').html(`<div class="text-muted small text-center py-2">No action links added. Click "Add Link" to add one.</div>`);
        }
...
$(document).ready(function() {
    $('#userMasteremailForm').on('submit', function(e) {
        e.preventDefault();
        ...
        $.ajax({
            url: "{{ route('user-master-sequences-list-update', $sequence->id) }}",
            method: 'POST',
            data: formData,
            processData: false,
            contentType: false,
