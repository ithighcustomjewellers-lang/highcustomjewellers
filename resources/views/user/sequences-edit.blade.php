                        ...
                        <form id="emailForm" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row g-3 mb-4">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <input type="number" name="gap_days" id="gapDays"
                                        ...
                                        value="{{ old('gap_days', $sequence->gap_days) }}">
                                </div>
                                <div class="col-md-3">
                                    <label class="form-label fw-semibold small text-uppercase text-muted">Variant
                                        (A/B/C)</label>
                                    <input type="text" name="variant" id="variant"
                                        class="form-control form-control-lg rounded-3 text-uppercase"
                                        value="{{ old('variant', $sequence->variant) }}" maxlength="1">
                                </div>
                                <div class="col-md-3">
                                    <label class="form-label fw-semibold small text-uppercase text-muted">Business
                                        Type</label>
                                    <select name="type" id="typeSelect"
                                        class="form-select form-control rounded-3 form-control-lg">
                                        <option value="B2B"
                                            {{ old('type', $sequence->type) == 'B2B' ? 'selected' : '' }}>B2B</option>
                                        <option value="B2C"
                                            {{ old('type', $sequence->type) == 'B2C' ? 'selected' : '' }}>B2C</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" name="subject" id="subject"
                                    ...
                                    value="{{ old('subject', $sequence->subject) }}">
                            </div>
                                    ...
                                    <div class="row g-3 align-items-end">
                                        <div class="col-md-7">
                                            ...
                                            <input type="hidden" name="existing_company_logo" id="existingCompanyLogo"
                                                value="{{ $sequence->existing_company_logo }}">
                                            <input type="hidden" name="image_type" id="imageType"
                                                value="{{ $sequence->image_type ?? 'logo' }}">
                                        </div>
                                        <div class="col-md-5">
                                            <div id="companyLogoPreview" class="mt-2"
                                                style="{{ $sequence->existing_company_logo ? 'display:block' : 'display:none' }}">
                                                <img id="companyLogoPreviewImg" class="img-fluid rounded-3 border"
                                                    style="max-height: 60px"
                                                    src="{{ $sequence->existing_company_logo ? asset($sequence->existing_company_logo) : '' }}">
                                            </div>
                                            <label class="form-label fw-semibold">Logo Position</label>
                                            <select name="logo_position" id="logoPosition" class="form-select">
                                                <option value="left"
                                                    {{ old('logo_position', $sequence->logo_position) == 'left' ? 'selected' : '' }}>
                                                    Left</option>
                                                <option value="center"
                                                    {{ old('logo_position', $sequence->logo_position) == 'center' ? 'selected' : '' }}>
                                                    Center</option>
                                                <option value="right"
                                                    {{ old('logo_position', $sequence->logo_position) == 'right' ? 'selected' : '' }}>
                                                    Right</option>
                                            </select>
                                        </div>
                                    </div>
                            ...
                            <div
                                ...
                                <div class="btn-group btn-group-sm">
                                    <button type="button" onclick="formatText('insertUnorderedList')"
                                        class="btn btn-outline-secondary">• List</button>
                                    <button type="button" onclick="formatText('insertOrderedList')"
                                        class="btn btn-outline-secondary">1. List</button>
                                </div>
                                <div class="btn-group btn-group-sm">
                            ...
                            <div class="mb-4">
                                ...
                                <div class="border rounded-3 p-3 bg-light">
                                    ...
                                    <div id="attachmentPreview" class="mt-2"
                                        style="{{ $sequence->attachments_image ? 'display:block' : 'display:none' }}">
                                        <div
                            ...
                            <div class="d-flex gap-3 mt-4 flex-wrap">
                                ...
                                <a href="{{ route('master-data-list') }}"
                                    class="btn btn-light btn-lg px-5 rounded-pill shadow-sm back-btn">
                    ...
                    <div class="card-body p-4 bg-light">
                        <div class="preview-container d-flex justify-content-center">
                            ...
                            <div id="mobilePreview" class="email-preview mobile-preview active"
                                style="width: 100%; max-width: 360px; background: white; border-radius: 32px; box-shadow: 0 20px 35px -12px rgba(0,0,0,0.3); overflow: hidden;">
                                ...
                                <div class="p-3" style="min-height: 520px;">
                                    <div class="email-preview-header bg-light p-2 rounded mb-2">
                                        <strong>Subject:</strong> <span
                                            id="previewSubject">{{ $sequence->subject }}</span>
                                    </div>
                            ...
                            <div id="desktopPreview" class="email-preview desktop-preview"
                                ...
                                <div class="bg-light px-4 py-2 border-bottom d-flex gap-2 align-items-center">
                                    ...
                                    <span class="ms-auto"><span
                                            id="previewSubjectDesktop">{{ $sequence->subject }}</span></span>
                                </div>
            ...
            if (file) {
                ...
                reader.onload = function(ev) {
                    ...
                    img.onload = () => {
                        ...
                        document.getElementById('removeLogoCheck') && (document.getElementById(
